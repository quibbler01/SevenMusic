...
object DecodeFormatManager {
    ...
    fun parseDecodeFormats(intent: Intent): Vector<BarcodeFormat?>? {
        var scanFormats: MutableList<String?>? = null
        val scanFormatsString = intent.getStringExtra(Intents.Scan.SCAN_FORMATS)
        if (scanFormatsString != null) {
            scanFormats = Arrays.asList<String?>(*COMMA_PATTERN.split(scanFormatsString))
        }
    ...
    fun parseDecodeFormats(inputUri: Uri): Vector<BarcodeFormat?>? {
        ...
        if (formats != null && formats.size == 1 && formats.get(0) != null) {
            formats = Arrays.asList<String?>(*COMMA_PATTERN.split(formats.get(0)))
        }
