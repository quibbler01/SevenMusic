...
object ActivityStart {
    fun startMvPlayActivity(context: Context, mvInfo: MvInfo?) {
        val intent = Intent(context, MvPlayActivity::class.java)
        intent.putExtra("mvInfo", mvInfo)
        context.startActivity(intent)
    }

    fun startMusicPlayActivity(context: Activity, mvMusicInfo: MvMusicInfo?) {
    ...
    fun startMusicPlayActivity(context: Context, mvMusicInfo: MvMusicInfo?) {
        val intent = Intent(context, MusicPlayActivity::class.java)
        intent.putExtra("musicInfo", mvMusicInfo)
        context.startActivity(intent)
        //进入播放页面的动画，从下往上
//        context.overridePendingTransition(R.anim.slide_from_top, R.anim.slide_to_bottom);
     ...
     * @param cls
     */
    fun startNewActivity(context: Context, cls: Class<*>?) {
