...
class TimingStopPlayDialog(context: Context) : View.OnClickListener {
     ...
     * 不开启
     */
    private var mSidebarTimingStopPlayCloseTv: TextView? = null

    /**
     * 10分钟后关闭
     */
    private var mSidebarTimingStopPlayCloseTenMinuteTv: TextView? = null

    /**
     * 20分钟后关闭
     */
    private var mSidebarTimingStopPlayCloseTwentyMinuteTv: TextView? = null

    /**
     * 30分钟后关闭
     */
    private var mSidebarTimingStopPlayCloseThirtyMinuteTv: TextView? = null

    /**
     * 40分钟后关闭
     */
    private var mSidebarTimingStopPlayCloseFortyMinuteTv: TextView? = null

    /**
     * 60分钟后关闭
     */
    private var mSidebarTimingStopPlayCloseSixtyMinuteTv: TextView? = null

    /**
     * 自定义时间
     */
    private var mSidebarTimingStopPlayCloseCustomMinuteTv: TextView? = null
     ...
     * Intent对象实例，用于传递数据
     */
    private val intent = Intent(MusicBroadcastManager.MUSIC_GLOBAL_TIMING_STOP_PLAY)
    ...
    init {
        ...
        mAlertDialog = AlertDialog.Builder(mContext).setView(initView()).create()
        mAlertDialog.show()
    }
    ...
    private fun initView(): View {
        val view = LayoutInflater.from(mContext)
            .inflate(R.layout.sidebar_timing_stop_play_dialog, null, false)

        mSidebarTimingStopPlayCloseTv =
            view.findViewById<TextView>(R.id.sidebar_tv_timing_stop_play_close)
        mSidebarTimingStopPlayCloseTenMinuteTv =
            view.findViewById<TextView>(R.id.sidebar_tv_timing_stop_play_close_ten_minute)
        mSidebarTimingStopPlayCloseTwentyMinuteTv =
            view.findViewById<TextView>(R.id.sidebar_tv_timing_stop_play_close_twenty_minute)
        mSidebarTimingStopPlayCloseThirtyMinuteTv =
            view.findViewById<TextView>(R.id.sidebar_tv_timing_stop_play_close_thirty_minute)
        mSidebarTimingStopPlayCloseFortyMinuteTv =
            view.findViewById<TextView>(R.id.sidebar_tv_timing_stop_play_close_forty_minute)
        mSidebarTimingStopPlayCloseSixtyMinuteTv =
            view.findViewById<TextView>(R.id.sidebar_tv_timing_stop_play_close_sixty_minute)
        mSidebarTimingStopPlayCloseCustomMinuteTv =
            view.findViewById<TextView>(R.id.sidebar_tv_timing_stop_play_close_custom_minute)
    ...
    override fun onClick(v: View) {
        ...
        if (v.getId() == R.id.sidebar_tv_timing_stop_play_close) {
            ...
            if (mSidebarTimingStopPlayCloseTenMinuteTv!!.getCompoundDrawables()[2] == null) {
                mSidebarTimingStopPlayCloseTenMinuteTv!!.setCompoundDrawables(
                    null,
                    null,
                    nav_right,
                    null
            ...
            mTime = (10 * 60 * 1000).toLong() // 10分钟（单位毫秒）
            close = ""
            mAlertDialog.dismiss()
            ...
            if (mSidebarTimingStopPlayCloseTwentyMinuteTv!!.getCompoundDrawables()[2] == null) {
                mSidebarTimingStopPlayCloseTwentyMinuteTv!!.setCompoundDrawables(
                    null,
                    null,
                    nav_right,
                    null
            ...
            mTime = (20 * 60 * 1000).toLong() //20分钟（单位毫秒）
            close = ""
            mAlertDialog.dismiss()
            ...
            if (mSidebarTimingStopPlayCloseThirtyMinuteTv!!.getCompoundDrawables()[2] == null) {
                mSidebarTimingStopPlayCloseThirtyMinuteTv!!.setCompoundDrawables(
                    null,
                    null,
                    nav_right,
                    null
            ...
            mTime = (30 * 60 * 1000).toLong() // 30分钟（单位毫秒）
            close = ""
            mAlertDialog.dismiss()
            ...
            if (mSidebarTimingStopPlayCloseFortyMinuteTv!!.getCompoundDrawables()[2] == null) {
                mSidebarTimingStopPlayCloseFortyMinuteTv!!.setCompoundDrawables(
                    null,
                    null,
                    nav_right,
                    null
            ...
            mTime = (40 * 60 * 1000).toLong() // 40分钟（单位毫秒）
            close = ""
            mAlertDialog.dismiss()
            ...
            if (mSidebarTimingStopPlayCloseSixtyMinuteTv!!.getCompoundDrawables()[2] == null) {
                mSidebarTimingStopPlayCloseSixtyMinuteTv!!.setCompoundDrawables(
                    null,
                    null,
                    nav_right,
                    null
            ...
            mTime = (60 * 60 * 1000).toLong() // 60分钟（单位毫秒）
            close = ""
            mAlertDialog.dismiss()
        ...
        if (ResUtil.getString(R.string.str_timing_stop_play_close) == close) {
            Toast.makeText(
                mContext,
                ResUtil.getString(R.string.str_timing_stop_play_cancel),
                Toast.LENGTH_SHORT
            ).show()
        } else {
            Toast.makeText(
                mContext,
                ResUtil.getString(R.string.str_timing_stop_play_set_success_1) + mTime / (60 * 1000) + ResUtil.getString(
                    R.string.str_timing_stop_play_set_success_2
                ),
                Toast.LENGTH_SHORT
            ).show()
        }
     ...
     * 描述：设置自定义定时关闭时间
     */
    private fun showCustomMinuteDialog() {
        val calendar = Calendar.getInstance() // 通过Calendar对象获取时、分信息
        //        hour = calendar.get(Calendar.HOUR_OF_DAY);
//        minute = calendar.get(Calendar.MINUTE);
        // 时间选择对话框
        val mTimePickDialog =
            TimePickerDialog(mContext, AlertDialog.THEME_HOLO_LIGHT, object : OnTimeSetListener {
                override fun onTimeSet(view: TimePicker?, hourOfDay: Int, minuteOfHour: Int) {
                    calendar.set(Calendar.HOUR_OF_DAY, hourOfDay)
                    calendar.set(Calendar.MINUTE, minuteOfHour)
                    hour = calendar.get(Calendar.HOUR_OF_DAY)
                    minute = calendar.get(Calendar.MINUTE)
                    mTime = (hour * 60 * 60 * 1000 + minute * 60 * 1000).toLong() // 自定义选择的时间（单位毫秒）
                    intent.putExtra("close", close)
                    intent.putExtra("time", mTime)
                    MusicBroadcastManager.sendBroadcast(intent)
                    Toast.makeText(
                        mContext,
                        "设置成功，将于" + hour + "小时" + minute + "分钟后关闭",
                        Toast.LENGTH_SHORT
                    ).show()
                }
